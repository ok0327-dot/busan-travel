...
| **부산 콘텐츠 규모** | **4,181건** (전국 안에서 부산 부분, `allRegnCd=23`) | **13,876건** (이미지만, 부산 100%) |
...
| **라이센스** | 공공누리 1~4 mixed (1유형 다수 가정) | 공공누리 1~4 + 자유이용불가 mixed |
| **robots.txt** | ✅ `Disallow: /*search*` 만 (CDN 허용) | 미발견 (관례상 default 허용) |
| **Phase A 큐레이션 size** | 1,200 (4,181 의 29%) | 800 (13,876 의 6%) |
...
공공누리 제3유형: 출처표시 / ✅ 영리 / ❌ 변형 (그대로만)
공공누리 제2/4유형: 영리 또는 변형 금지 — 우리 affiliate 비호환 → SKIP
자유이용불가: SKIP
```

| Tier | 변형 정도 | KOGL-1 | KOGL-3 | 비고 |
|---|---|---|---|---|
| **HERO** (char_I 40~60% 인페인트) | 큰 변형 | ✅ | ❌ | 합성 input 절대 금지 (3유형) |
| **PW** (char_I 5~15% 인페인트) | 변형 있음 | ✅ | ❌ | 동일 |
| **DETAIL** (음식·소품 크롭) | 크롭 — KOGL 해석 모호 | ✅ | ⚠️ 보수 적용 | 안전하게 1유형만 |
| **RAW** (그대로 게시) | 변형 0 | ✅ | ✅ | ⭐ 풀 2배+ 확장 가능 |
...
2차: 부산 archive — 13K + 메타 풍부 (POI 매핑 강함) + 카테고리 보충
